...
public class ChickManager : MonoBehaviour
{
    ...
    public void ShowMoveOptions()
    {
        ...
        Vector3 moveDirection = owner == GameManager.Player.Player1 ? transform.right : -transform.right;

        Vector3 highlightPosition = currentPosition + moveDirection * moveDistance;

        // フィールド内にあり、味方の駒がいない場合にハイライトを生成
        if (gameManager.IsWithinField(highlightPosition) &&
            !gameManager.IsFriendlyPieceAt(highlightPosition, owner))
        {
